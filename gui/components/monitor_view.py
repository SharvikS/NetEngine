...
Both flows are theme-aware and integrate with the rest of Net Engine's
...
class MonitorView(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Flipped in shutdown() so every async worker callback
        # early-returns instead of touching deleted widgets.
    ...
    def _on_stats(self, target: str, sent: int, lost: int, last_rtt: str):
        ...
        try:
            ...
            # Defensive: every item could theoretically be None if
            # the row was rebuilt mid-update.
            it1 = self._table.item(row, 1)
            it2 = self._table.item(row, 2)
            it3 = self._table.item(row, 3)
            it4 = self._table.item(row, 4)
            if it1 is None or it2 is None or it3 is None or it4 is None:
                return
            it1.setText(str(sent))
            it2.setText(str(lost))
            it3.setText(f"{loss_pct:.1f}%")
            it4.setText(last_rtt)
    ...
    def shutdown(self):
        # Flip the flag first so any in-flight worker signals that
        # have already been queued will be dropped when they reach
        # our slots on the main thread.
        self._shutting_down = True

        # Disconnect every worker signal BEFORE telling the worker
        # to stop. Once the signals are broken the worker can emit
        # freely without landing in a deleted slot.
        for w in list(self._workers.values()):
            disconnect_signal(getattr(w, "stats", None))
            disconnect_signal(getattr(w, "finished_target", None))
            disconnect_signal(getattr(w, "line", None))
            try:
                w.stop()
            except Exception:
                pass
