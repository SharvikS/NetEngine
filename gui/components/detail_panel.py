...
class DetailPanel(QFrame):
    ...
    def _populate(self, host: HostInfo) -> None:
        ...
        try:
            t = theme()
            self._hdr.setText(host.ip or "—")

            self._f_ip.set_value(host.ip or "—")
            ...
            self._f_status.set_value(
                (host.status or "").upper() or "—",
                color=status_color,
            )
            ...
            ports_str = describe_ports(open_ports) if open_ports else "None found"
            ...
            self._in_host.setText(host.ip or "")
            if not self._in_name.text().strip() or self._in_name.property("auto"):
                self._in_name.setText(f"Scan-{host.ip or 'host'}")
    ...
    def _clear_copy_ip_label(self) -> None:
        """
        Reset the Copy-IP button label after the 1500ms feedback
        window. Used via QTimer.singleShot; guarded so closing the
        drawer mid-feedback can never poke a deleted QPushButton.
        """
        try:
            self._btn_copy_ip.setText("Copy IP")
        except RuntimeError:
            return
