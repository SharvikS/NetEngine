...
class AppStatusBar(QStatusBar):
    ...
    def __init__(self, parent=None):
        ...
        # Flipped in shutdown() so timer ticks don't touch child
        # labels that may already have been cascade-deleted by the
        # main window's closeEvent teardown.
    ...
    def shutdown(self) -> None:
        """
        Called before the main window tears down. Stops every timer
        so no tick can fire into a cascade-deleted label. Safe to
        call repeatedly.
        """
        self._shutting_down = True
        for timer_attr in ("_transient_timer", "_clock_timer", "_health_timer"):
