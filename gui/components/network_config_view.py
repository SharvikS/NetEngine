...
class NetworkConfigView(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Flipped in shutdown() so the netsh completion callback
        # drops silently after the view has been torn down.
        self._shutting_down = False
        self._completion: tuple[str, str] | None = None
        self._build_ui()
    ...
    def _on_apply(self):
        ...
        if not net_config.is_admin():
            QMessageBox.warning(
                ...
                "Re-launch Net Engine as Administrator and try again."
            )
            return
    ...
    def _run_in_thread(self, func, adapter):
        ...
        def worker():
            ...
            # If the view was torn down while netsh was running, do
            # not bother bouncing back to the main thread — the slot
            # would fire on a destroyed widget and crash.
            if self._shutting_down:
                return
    ...
    def _apply_completed(self):
        if self._shutting_down:
            return
        kind, msg = self._completion or ("error", "?")
        try:
    ...
    def shutdown(self):
        """Called by MainWindow before teardown."""
