...
class ApiConsoleView(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Flipped in shutdown() so a late HTTP response is dropped
        # instead of trying to write into deleted QPlainTextEdits.
    ...
    def _on_send(self):
        ...
        # If an earlier request is still alive, disconnect it so its
        # late result can't overwrite the new one's UI state. The old
        # QThread is left to finish naturally.
        if self._worker is not None:
            disconnect_signal(getattr(self._worker, "done", None))
